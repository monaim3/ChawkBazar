...
export const ProductAttributes: React.FC<Props> = ({
  ...
  return (
    <div className={className}>
      ...
      <ul className="flex flex-wrap ltr:-mr-3 rtl:-ml-3">
        {attributes?.map(({ id, value, colorCode }) => (
          <li
            key={`${value}-${id}`}
            className={cn(
              "cursor-pointer rounded border w-9 md:w-11 h-9 md:h-11 p-1 mb-2 md:mb-3 ltr:mr-2 rtl:ml-2 flex justify-center items-center transition duration-200 ease-in-out hover:border-black",
              value === active ? "border-black" : "border-gray-100"
            )}
            onClick={() => onClick({ [title]: value })}
          >
